class VamtamImage extends elementorModules.frontend.handlers.Base {
    ...
    animateGrow() {
        ...
        $imageWrap.removeClass(animation), this.currentAnimation && $imageWrap.removeClass(this.currentAnimation), this.currentAnimation = animation, animationDuration ? ($imageWrap[0].style["-webkit-animation-duration"] = this.animDuration + "s", $imageWrap[0].style["-moz-animation-duration"] = this.animDuration + "s", $imageWrap[0].style["-o-animation-duration"] = this.animDuration + "s", $imageWrap[0].style["animation-duration"] = this.animDuration + "s") : (this.$element.hasClass("animated-slow") && $imageWrap.addClass("animated-slow"), this.$element.hasClass("animated-fast") && $imageWrap.addClass("animated-fast")), setTimeout(() => {
    ...
    animateScale() {
        const $image = this.elements.$image,
            animation = this.getAnimation();
        if ("none" === animation) return void $image.removeClass("elementor-invisible");
        const elementSettings = this.getElementSettings(),
            animationDelay = elementSettings._animation_delay || elementSettings.animation_delay || 0,
            animationDuration = this.animDuration || elementSettings.animation_duration || "";
        $image.removeClass(animation), this.currentAnimation && $image.removeClass(this.currentAnimation), this.currentAnimation = animation, animationDuration ? ($image[0].style["-webkit-animation-duration"] = animationDuration + "s", $image[0].style["-moz-animation-duration"] = animationDuration + "s", $image[0].style["-o-animation-duration"] = animationDuration + "s", $image[0].style["animation-duration"] = animationDuration + "s") : (this.$element.hasClass("animated-slow") && $image.addClass("animated-slow"), this.$element.hasClass("animated-fast") && $image.addClass("animated-fast")), setTimeout(() => {
            window.requestAnimationFrame(() => {
...
jQuery(window).on("elementor/frontend/init", () => {
    if (elementorFrontend.elementsHandler && elementorFrontend.elementsHandler.attachHandler) elementorFrontend.elementsHandler.attachHandler("image", VamtamImage);
    else {
